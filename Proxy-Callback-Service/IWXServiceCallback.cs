...
namespace PlatformWcfServers
{
    ...
    public interface IWXServiceCallback
    {
        /// <summary>
        /// 测试代理连接是否正常
        /// </summary>
        ...
        /// <returns></returns>
        [OperationContract]
        string MonthlyRenewalsCallback(string CardID, string PKID, int MonthNum, decimal Amount, string AccountID, int PayWay, int OrderSource, string OnlineOrderID, DateTime PayDate);
        ...
        /// <returns></returns>
        [OperationContract]
        string GetMonthCardCallback(string city_id, string parking_id, string license_plate, int page, int per_page);
        #endregion

        #region 中兴定制
        [OperationContract]
        string IssuedZXCallback(string IORec, string PKID, string carNumber, int amout);
        #endregion
        [OperationContract(IsOneWay = true)]
        void NotifyReceived(string title, string message);
    }
}