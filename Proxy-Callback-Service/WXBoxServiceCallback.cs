...
namespace PlatformWcfServers
{
    ...
    public class WXBoxServiceCallback
    {
        ...
        /// 日志记录对象
        /// </summary>
        protected Logger logger = LogManager.GetLogger("微信岗亭回调服务");
        ...
        /// 回调集合
        /// </summary>
        private static ConcurrentDictionary<string, WXBoxClient> dit_callback = new ConcurrentDictionary<string, WXBoxClient>();
        ...
        public bool TestClientProxyConnection(string ProxyNo)
        {
            if (dit_callback.ContainsKey(ProxyNo))
            {
                try
                {
                    dit_callback[ProxyNo].callback.TestClientProxyConnectionCallBack();
        ...
        static void timer1_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            //{
            //    WXBoxClient client = null;

            //    if(dit_callback.TryGetValue(key, out client) && client.updatetime.AddSeconds(30) < DateTime.Now)
            //    {
            ...
            foreach (var key in dit_callback.Keys)
            {
                ...
                if (dit_callback.TryGetValue(key, out client))
                {
                    try
                    {
        ...
        public void Update(string ProxyNo)
        {
            ...
            if(dit_callback.TryGetValue(ProxyNo, out client))
            {
                ...
                client.callback = OperationContext.Current.GetCallbackChannel<IWXBoxServiceCallback>();
            }
            else
            {
                try
