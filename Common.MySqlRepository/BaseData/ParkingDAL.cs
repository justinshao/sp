...
namespace Common.MySqlRepository
{
    public class ParkingDAL : BaseDAL, IParking
    {
        ...
        public bool Add(BaseParkinfo model, DbOperator dbOperator)
        {
            ...
            strSql.Append(",LastUpdateTime,HaveUpdate,CityID,VID,DataStatus,DataSaveDays,PictureSaveDays,IsOnLineGathe,IsLine,NeedFee,ExpiredAdvanceRemindDay,DefaultPlate,PoliceFree,UnconfirmedCalculation,IsNoPlateConfirm,SupportAutoRefund)");
            ...
            strSql.Append(",@IsOnLineGathe,@IsLine,@NeedFee,@ExpiredAdvanceRemindDay,@DefaultPlate,@PoliceFree,@UnconfirmedCalculation,@IsNoPlateConfirm,@SupportAutoRefund)");
        ...
        public bool Update(BaseParkinfo model)
        {
            using (DbOperator dbOperator = ConnectionManager.CreateConnection())
            {
                ...
                strSql.Append(",CarBitNumFixed=@CarBitNumFixed,CarBitNumLeft=@CarBitNumLeft,OnlineDiscount=@OnlineDiscount,IsOnlineDiscount=@IsOnlineDiscount,UnconfirmedCalculation=@UnconfirmedCalculation,IsNoPlateConfirm=@IsNoPlateConfirm,SupportAutoRefund=@SupportAutoRefund where  PKID=@PKID");
                dbOperator.ClearParameters();
                dbOperator.AddParameter("PKID", model.PKID);
                dbOperator.AddParameter("PKNo", model.PKNo);
                dbOperator.AddParameter("PKName", model.PKName);
