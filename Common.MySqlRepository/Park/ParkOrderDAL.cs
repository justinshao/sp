    ...
    public class ParkOrderDAL : BaseDAL, IParkOrder
    {
        ...
        public ParkOrder GetIORecordOrderChareFeeCount(string userID, OrderType orderType, DateTime dt, int releaseType, out string ErrorMessage)
        {
            ...
            try
            {
                ...
                string strSql = string.Format(@"SELECT sum(O.Amount) as Amount,sum(O.PayAmount) as PayAmount,sum(O.DiscountAmount) as DiscountAmount  FROM  ParkOrder  O  
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select sum(o.PayAmount) PayAmount from  ParkIORecord p inner join ParkOrder o on p.RecordID=o.TagID");
