...
namespace Common.MySqlRepository
{
    public class ParkCarTypeDAL : BaseDAL, IParkCarType
    {
        ...
        public bool Add(List<ParkCarType> models, DbOperator dbOperator)
        {
            ...
            strSql.Append(",OverdueToTemp,LotOccupy,Deposit,MonthCardExpiredEnterDay,AffirmBegin,AffirmEnd,IsNeedCapturePaper,IsNeedAuthentication,IsDispatch)");
            bool hasData = false;
            int index = 1;
            foreach (var p in models)
            {
                ...
                strSql.AppendFormat(",@OverdueToTemp{0},@LotOccupy{0},@Deposit{0},@MonthCardExpiredEnterDay{0},@AffirmBegin{0},@AffirmEnd{0},@IsNeedCapturePaper{0},@IsNeedAuthentication{0},@IsDispatch{0}  UNION ALL", index);
        ...
        public bool Update(ParkCarType model)
        {
            ...
            strSql.Append(",OverdueToTemp=@OverdueToTemp,LotOccupy=@LotOccupy,Deposit=@Deposit,MonthCardExpiredEnterDay=@MonthCardExpiredEnterDay,AffirmBegin=@AffirmBegin,AffirmEnd=@AffirmEnd,IsNeedCapturePaper=@IsNeedCapturePaper,IsNeedAuthentication=@IsNeedAuthentication,IsDispatch=@IsDispatch");
            strSql.Append(" where CarTypeID=@CarTypeID");
            using (DbOperator dbOperator = ConnectionManager.CreateConnection())
            {
                dbOperator.ClearParameters();
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM ParkCarType");
