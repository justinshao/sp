...
namespace ServicesDLL.Model.KDE
{
    public class SignData
    {
        ...
        /// <param name="ConvertString"></param>
        /// <returns></returns>
        public static string GetHMACSHA256Str(string ConvertString)
        {
            
            HMACSHA256 provider = new HMACSHA256();
            byte[] bts= provider.ComputeHash(Encoding.UTF8.GetBytes(ConvertString));
            string ss = Convert.ToBase64String(bts); 
            return ss;
        }

        public static string GetHMACSHA256Str2(string key, string ConvertString)
        {

            HMACSHA256 provider = new HMACSHA256(Encoding.UTF8.GetBytes(key));
            byte[] bts = provider.ComputeHash(Encoding.UTF8.GetBytes(ConvertString));
            string ss = Convert.ToBase64String(bts);
            return ss;
        }
