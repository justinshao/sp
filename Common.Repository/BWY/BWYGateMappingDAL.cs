    ...
    public class BWYGateMappingDAL : BaseDAL, IBWYGateMapping
    {
        public bool Add(BWYGateMapping model)
        {
            using (DbOperator dbOperator = ConnectionManager.CreateConnection())
            {
                ...
                strSql.Append("insert into BWYGateMapping(RecordID,ParkingID,ParkingName,GateID,GateName,CreateTime,DataSource,DataStatus,LastUpdateTime,HaveUpdate)");
                strSql.Append(" values(@RecordID,@ParkingID,@ParkingName,@GateID,@GateName,@CreateTime,@DataSource,@DataStatus,@LastUpdateTime,@HaveUpdate)");
        ...
        public List<BWYGateMapping> QueryByDataSource(int dataSurce)
        {
            ...
            {
                dbOperator.ClearParameters();
                dbOperator.AddParameter("DataSource", dataSurce);
                dbOperator.AddParameter("DataStatus", (int)DataStatus.Delete);
                using (DbDataReader reader = dbOperator.ExecuteReader(strSql.ToString()))
                {
                    while (reader.Read())
