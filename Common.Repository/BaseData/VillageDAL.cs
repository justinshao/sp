...
namespace Common.SqlRepository
{
    public class VillageDAL : BaseDAL, IVillage
    {
        ...
        {
            string sql = "select * from BaseVillage where CPID=@CPID and DataStatus!=@DataStatus;";
            using (DbOperator dbOperator = ConnectionManager.CreateReadConnection())
            {
                dbOperator.ClearParameters();
                dbOperator.AddParameter("CPID", companyId);
        ...
        {
            string sql = "select * from BaseVillage where VID=@VID and DataStatus!=@DataStatus;";
            using (DbOperator dbOperator = ConnectionManager.CreateReadConnection())
            {
                dbOperator.ClearParameters();
        ...
        {
            string sql = string.Format("select * from BaseVillage where VID in('{0}') and DataStatus!=@DataStatus;",string.Join("','",recordIds));
            using (DbOperator dbOperator = ConnectionManager.CreateReadConnection())
            {
                dbOperator.ClearParameters();
                dbOperator.AddParameter("DataStatus", (int)DataStatus.Delete);
        ...
        {
            string sql = "select * from BaseVillage where  DataStatus!=@DataStatus;";
            using (DbOperator dbOperator = ConnectionManager.CreateReadConnection())
            {
                dbOperator.ClearParameters();
                dbOperator.AddParameter("DataStatus", (int)DataStatus.Delete);
