...
namespace Common.SqlRepository
{
    public class ParkVisitorDAL : BaseDAL, IParkVisitor
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select a.*,b.UserName from VisitorInfo a left join SysUser b on a.OperatorID=b.RecordID where a.DataStatus!=2 and a.CreateTime>=@StartTime and a.CreateTime<=@EndTime ");
